...
export default function AdminPage() {
  ...
  return <DashboardLayout>{loading ? null : user?.role === "admin" ? <AdminWorkspace /> : <div className="p-8"><p className="eyebrow">KHU VỰC HẠN CHẾ</p><h1 className="mt-3 text-3xl font-extrabold text-[#092C5C]">Bạn chưa có quyền quản trị 4T.</h1><p className="mt-3 text-sm text-[#61718A]">Chỉ tài khoản chủ sở hữu dự án được truy cập bảng điều hành.</p></div>}</DashboardLayout>;
}
      ...
      <section className="rounded-2xl border border-[#092C5C]/10 bg-white p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-[#6D7F9B]">Cập nhật giá theo kg để đồng bộ lên giao diện khách hàng và đơn hàng mới.</p>
          </div>
